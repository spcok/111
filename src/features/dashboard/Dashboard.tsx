...
export function Dashboard() {
  ...
  // --- 1. Robust RxDB Read Pipeline ---
  const { data: allAnimals = [] } = useQuery<Animal[]>({ 
    queryKey: ['animals'],
    queryFn: async () => {
      const db = await getDB();
      const docs = await db.animals.find().exec();
  ...
  const { data: allLogs = [] } = useQuery<DailyLog[]>({
    ...
    queryFn: async () => {
      const db = await getDB();
      const docs = await db.daily_logs.find().exec();
      return docs.map((d: any) => d.toJSON());
    }
  });
  ...
  const { data: allSchedules = [] } = useQuery<FeedingSchedule[]>({
    ...
    queryFn: async () => {
      const db = await getDB();
      const docs = await db.feeding_schedules.find().exec();
      return docs.map((d: any) => d.toJSON());
    }
  });
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-3">
        <div>
          ...
          <p className="text-slate-400 mt-1 flex items-center gap-2 text-xs font-bold uppercase tracking-widest">
            RxCore Storage <span className="text-slate-700">|</span> 🌤️ IndexedDB
          </p>
        </div>
      </div>
